  ...
  return (
    <Card id={id}>
      ...
      <CardContent sx={{ p: 3.5 }}>
        ...
        {/* Title */}
        <Typography variant="h5" fontWeight={700} lineHeight={1.35} sx={{ mb: 3 }}>
          {pub.title}
        </Typography>

        {/* Authors */}
        <Stack direction="row" flexWrap="wrap" spacing={2} sx={{ mb: 3 }} alignItems="center">
          {pub.authors.map((a) => (
            <AuthorMedium key={a.id} author={a} />
          ))}
        </Stack>
        ...
        {keywords.length > 0 && (
          <Stack direction="row" alignItems="flex-start" gap={1} mb={2}>
            <Typography variant="body2" color="text.secondary" fontWeight={500} sx={{ lineHeight: '24px', whiteSpace: 'nowrap' }}>
              Keywords:
            </Typography>
          ...
          <Stack direction="row" flexWrap="wrap" gap={2}>
            {pub.doi && (
              <Typography variant="body2" color="text.secondary">
                DOI:{' '}
                <Typography component="a" href={`https://doi.org/${pub.doi}`} target="_blank" rel="noreferrer"
                  variant="body2" color="primary" sx={{ textDecoration: 'none', '&:hover': { textDecoration: 'underline' } }}>
                  {pub.doi}
                </Typography>
              </Typography>
            )}
            ...
            {pub.url && (
              <Typography component="a" href={pub.url} target="_blank" rel="noreferrer"
